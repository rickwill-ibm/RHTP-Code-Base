...
const PROGRAM_MOD: Record<string, { closureBoost: number; gainMod: number; operationalMod: number }> = {
  'RHTP — Medicaid 1115 Waiver':        { closureBoost: 0,    gainMod: 1.00, operationalMod: 1.00 },
...
const ORG_DATA: Record<string, { closure: number; gainShare: number; patients: number; type: string }> = {
...
function useDerivedData(
  ...
  return useMemo(() => {
    ...
    const scaledGapsClosed = Math.round(BASE.gapsClosed * (region === 'All Regions' ? 1 : regionScale) * periodMult);
    const scaledGapsOpen   = Math.round(BASE.gapsOpen   * (region === 'All Regions' ? 1 : regionScale) * periodMult);
    const closureRate      = Math.min(99, +(BASE.closureRate + progMod.closureBoost + (region === 'All Regions' ? 0 : (regionScale - 0.17) * 15)).toFixed(1));
    const gainShareKtd     = Math.round(BASE.gainShare * (region === 'All Regions' ? 1 : regionScale) * progMod.gainMod * periodMult);
    const sharedSavings    = Math.round(BASE.sharedSavings * (region === 'All Regions' ? 1 : regionScale) * progMod.gainMod * periodMult);
    const incentivePayments = Math.round(BASE.incentivePayments * (region === 'All Regions' ? 1 : regionScale) * periodMult);
    const avoidedLeakage   = Math.round(BASE.avoidedLeakage * (region === 'All Regions' ? 1 : regionScale) * periodMult);
    ...
    const providerPart    = +(Math.min(99, BASE.providerParticipation * (region === 'All Regions' ? 1 : 0.95 + regionScale))).toFixed(0);
    const ptEngagement    = +(BASE.patientEngagement * progMod.operationalMod * (region === 'All Regions' ? 1 : 0.92 + regionScale)).toFixed(0);

    // Closure trend — scale & shift per filters
    const closureTrend = [
      { month: 'Jan',        closed: Math.round(820  * regionScale * periodMult), open: Math.round(9200 * regionScale), rate: +(58 + progMod.closureBoost).toFixed(1) },
      { month: 'Feb',        closed: Math.round(940  * regionScale * periodMult), open: Math.round(9100 * regionScale), rate: +(61 + progMod.closureBoost).toFixed(1) },
      { month: 'Mar',        closed: Math.round(1100 * regionScale * periodMult), open: Math.round(8900 * regionScale), rate: +(63 + progMod.closureBoost).toFixed(1) },
      { month: 'Apr',        closed: Math.round(1280 * regionScale * periodMult), open: Math.round(8600 * regionScale), rate: +(65 + progMod.closureBoost).toFixed(1) },
      { month: 'May',        closed: Math.round(1420 * regionScale * periodMult), open: Math.round(8241 * regionScale), rate: closureRate },
      { month: 'Jun (proj)', closed: Math.round(1580 * regionScale * periodMult), open: Math.round(7900 * regionScale), rate: Math.min(99, +(closureRate + 2.5).toFixed(1)) },
    ];

    const gainShareTrend = [
      { month: 'Jan', earned: Math.round(180 * regionScale * progMod.gainMod * periodMult), projected: Math.round(200 * regionScale * progMod.gainMod) },
      { month: 'Feb', earned: Math.round(210 * regionScale * progMod.gainMod * periodMult), projected: Math.round(230 * regionScale * progMod.gainMod) },
      { month: 'Mar', earned: Math.round(240 * regionScale * progMod.gainMod * periodMult), projected: Math.round(260 * regionScale * progMod.gainMod) },
      { month: 'Apr', earned: Math.round(230 * regionScale * progMod.gainMod * periodMult), projected: Math.round(280 * regionScale * progMod.gainMod) },
      { month: 'May', earned: Math.round(240 * regionScale * progMod.gainMod * periodMult), projected: Math.round(300 * regionScale * progMod.gainMod) },
      { month: 'Jun (proj)', earned: 0, projected: Math.round(320 * regionScale * progMod.gainMod) },
    ];
    ...
    const measures = BASE_MEASURES
      .filter(m => !programMeasureKeys || programMeasureKeys.includes(m.measure))
      .map(m => ({ ...m, current: Math.min(99, Math.max(30, Math.round(m.current + regionAdj + progMod.closureBoost / 2))) }));

    // Network table — filter by org or show all
    const networkRows = org === 'All Organizations'
    ...
    return {
      scaledLives, scaledGapsClosed, scaledGapsOpen, closureRate,
      gainShareKtd, sharedSavings, incentivePayments, avoidedLeakage,
      refCompletion, responseTime, providerPart, ptEngagement,
      closureTrend, gainShareTrend, measures, networkRows,
      benchmarkPmpm, actualPmpm, savingsAnnual,
    };
...
export default function ExecutiveOutcomesDashboardPage() {
  ...
  // ── Metric cards helper
  const MetricCard = ({
    label, value, sub, color, icon, trend, up,
  }: { label: string; value: string; sub: string; color: string; icon: string; trend: string; up: boolean }) => (
  ...
  const isFiltered = region !== 'All Regions' || program !== 'All Programs' || org !== 'All Organizations' || period !== 'YTD 2026';
  ...
  return (
      ...
      contextBanner={
        <div className="bg-[#161616] border-b border-carbon-gray-80 px-6 py-2 flex items-center gap-6 flex-wrap">
          ...
          <span className="text-xs text-carbon-gray-30">
            {region === 'All Regions' ? '14 Counties' : region} · {d.scaledLives.toLocaleString()} Lives
    ...
    >
      ...
      <div className="flex items-center gap-4 px-6 py-2.5 border-b border-carbon-gray-20 bg-carbon-gray-10 flex-wrap">
        ...
        {isFiltered && (
          <button
            onClick={() => { setRegion(REGIONS[0]); setProgram(PROGRAMS[0]); setOrg(ORGS[0]); setPeriod(PERIODS[0]); }}
      ...
      <div className="flex items-center gap-3 px-6 py-3 border-b border-carbon-gray-20 bg-white flex-wrap">
        <span className="text-2xs font-semibold text-carbon-gray-50 uppercase tracking-wide mr-1">Filter:</span>
        {[
      ...
      <div className="px-6 py-4 space-y-4">
        ...
        {activeSection === 'population' && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MetricCard label="Care Gaps Closed" value={d.scaledGapsClosed.toLocaleString()} sub={period} color="text-[#24a148]" icon="CheckCircleIcon" trend={`${Math.round(d.scaledGapsClosed * 0.18).toLocaleString()} vs prior period`} up={true} />
              <MetricCard label="Open Care Gaps"   value={d.scaledGapsOpen.toLocaleString()}   sub="Remaining"     color="text-[#da1e28]" icon="ExclamationTriangleIcon" trend={`${Math.round(d.scaledGapsOpen * 0.07).toLocaleString()} closed this quarter`} up={false} />
              <MetricCard label="Closure Rate"     value={`${d.closureRate}%`} sub="Target: 75%" color="text-[#b45309]" icon="ChartBarIcon" trend={`${d.closureRate >= 68 ? '+' : ''}${(d.closureRate - 64).toFixed(1)}% vs Q1`} up={d.closureRate >= 64} />
              <MetricCard label="Quality Score"    value={`${(3.8 + (d.closureRate - 68.4) * 0.02).toFixed(1)}★`} sub="STARS Rating" color="text-[#f1c21b]" icon="StarIcon" trend="+0.3 vs prior year" up={true} />
            </div>
              ...
              <div className="flex items-center justify-between mb-4">
                <div>
                  ...
                  <p className="text-xs text-carbon-gray-50">
                    {region !== 'All Regions' ? region : 'All Regions'} · {program !== 'All Programs' ? program : 'All Programs'}
                  </p>
                </div>
        ...
        {activeSection === 'financial' && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MetricCard label="Gain Share Earned"   value={`$${(d.gainShareKtd / 1000).toFixed(2)}M`}  sub={period}                color="text-[#24a148]" icon="CurrencyDollarIcon" trend={`On track for $${(d.gainShareKtd * 1.67 / 1000).toFixed(2)}M annualized`} up={true} />
              <MetricCard label="Shared Savings"      value={`$${d.sharedSavings}K`}     sub="Realized"             color="text-[#24a148]" icon="BanknotesIcon"      trend={`+$${Math.round(d.sharedSavings * 0.15)}K vs prior period`} up={true} />
              <MetricCard label="Incentive Payments"  value={`$${d.incentivePayments}K`}  sub="Quality bonuses"      color="text-[#0043ce]" icon="TrophyIcon"         trend="Q2 payment pending" up={true} />
              <MetricCard label="Avoided Leakage"     value={`$${d.avoidedLeakage}K`}    sub="Out-of-network saves" color="text-[#6929c4]" icon="ShieldCheckIcon"    trend="-18% leakage rate" up={false} />
            </div>
            ...
            <div className="bg-white border border-carbon-gray-20 px-5 py-4">
              <h3 className="text-sm font-semibold text-carbon-gray-100 mb-4">
                ...
                {isFiltered && <span className="text-carbon-gray-50 font-normal text-xs ml-2">(adjusted for {region !== 'All Regions' ? region : program})</span>}
              </h3>
              <div className="grid grid-cols-3 gap-4">
                {[
                  { label: 'Benchmark PMPM', value: `$${d.benchmarkPmpm}`, sub: 'State-set target',         color: 'bg-carbon-gray-10 border-carbon-gray-20' },
                  { label: 'Actual PMPM',    value: `$${d.actualPmpm}`,    sub: `$${d.benchmarkPmpm - d.actualPmpm} below benchmark`, color: 'bg-[#defbe6] border-[#a7f0ba]' },
                  { label: 'Savings Generated', value: `$${(d.savingsAnnual / 1000).toFixed(2)}M`, sub: 'Annualized projection', color: 'bg-[#defbe6] border-[#a7f0ba]' },
                ].map(item => (
        ...
        {activeSection === 'operational' && (
          <>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              <MetricCard label="Referral Completion"       value={`${d.refCompletion}%`}  sub="Specialist tasks"     color="text-[#0043ce]" icon="ArrowsRightLeftIcon" trend="+6% vs prior period" up={true} />
              <MetricCard label="Specialist Response Time"  value={`${d.responseTime}d`}   sub="Avg days to accept"   color="text-[#24a148]" icon="ClockIcon"           trend="-0.8d vs Q1"       up={false} />
              <MetricCard label="Provider Participation"    value={`${d.providerPart}%`}   sub="Active in network"    color="text-[#24a148]" icon="UserGroupIcon"       trend="+2 orgs joined"    up={true} />
              <MetricCard label="Patient Engagement"        value={`${Math.min(99, d.ptEngagement)}%`} sub="Outreach response" color="text-[#b45309]" icon="PhoneIcon" trend="+3% vs Q1" up={true} />
            </div>
                ...
                <table className="w-full text-xs">
                  <thead>
                    <tr className="bg-carbon-gray-10 border-b border-carbon-gray-20">
                      ...
                      <th className="px-4 py-2.5 text-right text-2xs font-semibold text-carbon-gray-50 uppercase tracking-wide">Gain Share {period}</th>
                  ...
                  <tbody className="divide-y divide-carbon-gray-20">
                    {d.networkRows.map(row => (
                      <tr key={row.org} className="hover:bg-carbon-gray-10 transition-colors">
                        ...
                        <td className="px-4 py-3 text-right font-mono text-carbon-gray-100">{row.patients.toLocaleString()}</td>
                        <td className="px-4 py-3 text-right">
                          <span className={`font-bold font-mono ${row.closure >= 70 ? 'text-[#24a148]' : row.closure >= 60 ? 'text-[#b45309]' : 'text-[#da1e28]'}`}>
                            {row.closure}%
                          </span>
                        </td>
                        <td className="px-4 py-3 text-right font-bold font-mono text-[#24a148]">${row.gainShare}K</td>
                        <td className="px-4 py-3 text-center">
                          <span className={`text-2xs font-semibold px-2 py-0.5 ${
                            row.closure >= 70 ? 'bg-[#defbe6] text-[#0e6027]' : 'bg-[#fdf6dd] text-[#b45309]'
                          }`}>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-carbon-gray-70 leading-relaxed">
                    ...
                    {isFiltered && (
                      <> Metrics above reflect the <strong>{region !== 'All Regions' ? region : 'all-region'}</strong>{' '}
                      {program !== 'All Programs' ? <>, <strong>{program}</strong></> : ''} view for <strong>{period}</strong>.</>
                    )}
                  </p>
                </div>
              </div>
