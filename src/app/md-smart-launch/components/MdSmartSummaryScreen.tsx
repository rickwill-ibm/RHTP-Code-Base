...
export default function MdSmartSummaryScreen({ launchContext, onAuditEntry, onOpenOrderEntry }: MdSmartSummaryScreenProps) {
  ...
  console.log('≡ƒöì DEBUG - Launch Context Patient ID:', launchContext.patientId);
  console.log('≡ƒöì DEBUG - Registry Patient Found:', registryPatient ? 'YES' : 'NO');
  console.log('≡ƒöì DEBUG - Registry Patient Name:', registryPatient?.name);
  console.log('≡ƒöì DEBUG - Registry Care Gaps Count:', registryPatient?.careGaps?.length);
            ...
            <div className="px-4 py-3 flex items-start gap-3 border-b border-carbon-gray-20">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 flex-wrap mb-1">
                  ...
                  <span className="text-2xs font-semibold px-1.5 py-0.5 bg-[#f6f2ff] text-[#6929c4] border border-[#d4bbff]">ΓÜí Cerner</span>
                </div>
                  ...
                  return (
                    <React.Fragment key={phase.key}>
                      ...
                      >
                        ...
                        {isActive && (
                          <span className="text-2xs text-[#da1e28] font-bold mt-0.5">47d Γû╝</span>
                        )}
                      </div>
                ...
                <div className="bg-red-50 border border-red-200 px-4 py-3 mb-3 shadow-sm">
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      ...
                      <p className="text-2xs text-gray-700 leading-relaxed mb-2">Recent ER utilization, deteriorating chronic condition control, and multiple open care gaps indicate elevated near-term risk. Trajectory: <span className="font-semibold text-red-700">Worsening Γåô</span></p>
                  ...
                  {generatedPlan ? (
                    <>
                      <p className="text-sm mb-4 opacity-90">
                        Comprehensive care plan created with {generatedPlan.goals?.length || 0} goals and {generatedPlan.goals?.reduce((sum, g) => sum + (g.interventions?.length || 0), 0) || 0} interventions. View in Patient History ΓåÆ Care Plans section.
                      </p>
                    ...
                    <>
                      ...
                      <div className="flex items-center gap-2 text-xs opacity-75">
                        ...
                        <span>3-Click Workflow: Generate ΓåÆ Review ΓåÆ Approve & Send</span>
                      </div>
                    </>
                  )}
                </div>
