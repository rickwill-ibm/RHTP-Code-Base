'use client';
/**
 * DtrTreeView — Step 3: Medical Necessity Match (Documentation Templates & Rules).
 * Two-column: Policy Requirement (left) vs. Patient Record Match (right).
 * Ported from PA-Standalone-SmartApp.
 */
import { useRef } from 'react';
import { usePaStore } from '@/lib/pa/usePaStore';
import PolicyRuleSummary from '@/components/pa/PolicyRuleSummary';
import type { DtrGroup, DtrLeaf, DtrResultEntry } from '@/lib/pa/pa-types';
...
export default function DtrTreeView() {
  ...
  if (dtrLoading) {
    return (
      <div>
        <h1 className="text-2xl font-bold text-gray-900 mb-5">DTR — Medical Necessity Match</h1>
        <div className="rounded-xl border border-gray-200 bg-white p-12 shadow-sm flex flex-col items-center gap-4 text-gray-400">
          ...
          <p className="text-sm font-semibold">Fetching Questionnaire Package(s) and matching criteria…</p>
          <p className="text-xs">Evidence merged from EMR and Payer Patient Access API</p>
        </div>
      </div>
    );
  }
    ...
    return (
      <div>
        ...
        <div className="rounded-xl border border-gray-200 bg-gray-50 p-8 text-center text-sm text-gray-500">
          <p className="mb-4">No DTR results yet — complete Steps 1–2 first (Order → CRD Checklist → Proceed to DTR).</p>
      ...
      <div className="flex items-center justify-between flex-wrap gap-4 rounded-xl border border-gray-200 bg-white p-5 shadow-sm">
        <p className="text-sm text-gray-500 max-w-lg">
          {allMet
            ...
            : 'Resolve remaining gaps by uploading supporting documentation to continue.'}
        </p>
        <button
      ...
      <div className={`flex items-center justify-between px-5 py-4 ${isMet ? 'bg-white' : 'bg-amber-50/60'}`}>
        <div className="flex items-center gap-3">
          ...
          {group.required === false && (
            <span className="rounded-full border border-gray-200 bg-gray-50 px-2 py-0.5 text-[10px] font-bold uppercase tracking-wide text-gray-500">Optional</span>
          )}
        </div>
        ...
        <div className="px-5 py-4">
          ...
          {!isMet && group.candidateCodes && (
            <div>
              ...
              <button onClick={onUpload} className="inline-flex items-center gap-2 rounded-lg border border-[#1669c1] px-4 py-2 text-xs font-bold text-[#1669c1] hover:bg-blue-50 transition-colors">
...
function EvidenceBlock({ leaf, uploadedEvidence }: { leaf: DtrLeaf; uploadedEvidence?: string }) {
  const hasDate = !!(leaf.dateOfService || leaf.recordedDate);
  return (
    <div>
      <span className="inline-block rounded-md border border-gray-200 bg-white px-2 py-0.5 font-mono text-xs font-bold text-gray-800">{leaf.code}</span>
      <p className="text-sm text-gray-700 mt-1.5">{leaf.label}</p>
      {(hasDate || leaf.performerName) && (
        <dl className="mt-2 space-y-1">
          {leaf.dateOfService ? (
            <div className="flex gap-1.5 text-xs"><dt className="flex-shrink-0 font-semibold text-gray-600">Date of Service:</dt><dd className="text-gray-500">{fmtDate(leaf.dateOfService)}</dd></div>
          ) : leaf.recordedDate ? (
            <div className="flex gap-1.5 text-xs"><dt className="flex-shrink-0 font-semibold text-gray-600">Recorded:</dt><dd className="text-gray-500">{fmtDate(leaf.recordedDate)}</dd></div>
          ) : null}
          {leaf.performerName && (
            <div className="flex gap-1.5 text-xs"><dt className="flex-shrink-0 font-semibold text-gray-600">Performing Provider:</dt><dd className="text-gray-500">{leaf.performerName}</dd></div>
          )}
        </dl>
      )}
