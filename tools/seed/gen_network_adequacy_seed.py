...
Generate the Network Adequacy seed deterministically for TWO states:
  ...
  - GA (Atlanta metro + a rural county) ΓÇö the storyboard demo.
  - SD (Maria's state: Sioux Falls, Rapid City, Aberdeen, plus the Pine Ridge /
    Rosebud reservation counties) ΓÇö frontier access ties to the rural
...
specialists far away ΓåÆ severe, realistic gaps. Output feeds src/lib/networkAdequacy.
"""
import json
import os
...
COUNTIES = [
    # Georgia ΓÇö Atlanta metro + rural
    ...
    # South Dakota ΓÇö Maria's state
